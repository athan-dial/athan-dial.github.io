...
2. Find all .md files in publish_queue/ directory
3. For each file:
...
**CRITICAL simplification per planning context:** Quartz v4 natively supports [[wikilinks]], so do NOT rewrite wiki-style links. Only handle:
- Frontmatter cleanup
- Asset copying (images)
- Image embed path rewriting: `![[image.png]]` stays as-is (Quartz handles it) BUT standard markdown images `![alt](media/image.png)` need path update to `/static/image.png`
